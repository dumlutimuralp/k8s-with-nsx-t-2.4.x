...
Note : By default K8S does not deploy workloads on the Master node. Hence technically NSX components are not needed on the master node but only on the worker nodes when defaults are used. In our environment, for demonstration purposes NSX Components are installed on all K8S nodes including the master. 
